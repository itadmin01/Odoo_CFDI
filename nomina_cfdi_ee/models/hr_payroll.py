                ...
                if holiday:
                    ...
                    if contract.septimo_dia:
                        if holiday.holiday_status_id.name == 'FJS' or holiday.holiday_status_id.name == 'FI':
                            ...
                            leaves[holiday.holiday_status_id.name]['number_of_days'] += hours / 8.0
                            _logger.info("entro2 %s", holiday.holiday_status_id.name)
                    else:
                        leave_days += hours / 8.0
                        leaves[holiday.holiday_status_id.name]['number_of_days'] += hours / 8.0
                        _logger.info("entro3 %s", holiday.holiday_status_id.name)
 #                       _logger.info("horas %s", hours)
 #                       _logger.info("dias %s", hours/8.0)
                else:
            ...
            attendances['number_of_days'] = attendances['number_of_hours'] / 8.0
            number_of_days = 0
            #periodo para nómina quincenal
            if contract.periodicidad_pago == '04' and contract.tipo_pago == '01':
                total_days = attendances['number_of_days'] + leave_days
                if total_days != 15:
                ...
                if contract.sept_dia:
                   ...
                   res.append(septimo_dia)
            else:
                number_of_days = attendances['number_of_hours'] / 8.0
    ...
    def action_cfdi_nomina_generate(self):
        for payslip in self:
            ...
            if json_response.get('factura_xml'):
                ...
                report_data = Template.env['report'].get_pdf([self.id], 'nomina_cfdi_ee.report_payslip')
