...
class ReasonCancelation(models.TransientModel):
    ...
    def Confirmar(self):
        if self.env.context.get('active_id') and self.env.context.get('active_model') == "account.invoice":
            move_obj = self.env['account.invoice'].browse(self.env.context['active_id'])
            print(move_obj)
