...
class import_account_payment_from_xml(models.TransientModel):
    ...
    def import_xml_file_button_cargar(self):
        ...
        if invoice_id.move_type not in 'entry':
        ...
        efi_files = self.env['account.edi.document'].sudo().search([('move_id','=',invoice_id.id)])
        if not efi_files:
            file_name = invoice_id.name.replace('/', '_') + '.xml'
            attach = self.env['ir.attachment'].sudo().create({
                ...
                'description': _('Factura CFDI del documento %s.') % invoice_id.name,
            })
