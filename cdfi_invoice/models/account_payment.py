...
class AccountPayment(models.Model):
    ...
    invoice_id = fields.Many2one('account.invoice', string=_('Factura'))
	
    @api.depends('name')
    @api.one
    def _get_number_folio(self):
        if self.number:
            self.number_folio = self.number.replace('/','')
    
    @api.model
    def create(self, vals):
        if self._context.get('invoice_id', False):
            vals['invoice_id'] = self._context.get('invoice_id', False)
        res = super(AccountPayment, self).create(vals)
        if res.invoice_id:
            res.no_de_pago = len(res.invoice_id.payment_ids) + 1
            res.saldo_pendiente = res.invoice_id.residual
    ...
    def _compute_banco_receptor(self):
        if self.journal_id:
            bank_id = self.env['res.partner.bank'].search([('journal_id', '=', self.journal_id.id)], limit=1)
            if bank_id:
                self.banco_receptor = bank_id.bank_name
                self.rfc_banco_receptor = bank_id.bank_bic
                self.cuenta_beneficiario = bank_id.acc_number
    ...
    def to_json(self):
        if not self.company_id.archivo_cer:
            raise ValidationError(_('Archivo .cer path is missing.'))
        archivo_cer_file = open(self.company_id.archivo_cer, 'rb').read()
        if not self.company_id.archivo_key:
            raise ValidationError(_('Archivo .key path is missing.'))
        ...
        if self.invoice_id:		
            ...
            request_params = { 
                ...
                'invoice': {
                      ...
                      'folio_complemento': self.number.replace('/',''),
            ...
            request_params = { 
                ...
                'invoice': {
                      'tipo_comprobante': self.tipo_comprobante,
                      'folio_complemento': self.number.replace('CUST.IN','').replace('/',''),
                      'serie_complemento': self.company_id.serie_complemento,
                },
                'concept': {
                      'claveprodserv': '84111506',
    ...
    def complete_payment(self):
        for p in self:
            values = p.to_json()
            url = '%s%s' % (p.company_id.http_factura, '/payment?handler=OdooHandler33')
            ...
            if estado_pago == 'problemas_pago':
                raise ValidationError(_(json_response['problemas_message']))
    ...
    def _set_data_from_xml(self, xml_payment):
        ...
        if self.invoice_id:
           ...
           if self.moneda != 'MXN':
              if DoctoRelacionado.attrib['TipoCambioDR']:
...
class MailTemplate(models.Model):
    ...
    def generate_email_batch(self, template_id, res_ids, fields=None):
        results = super(MailTemplate, self).generate_email_batch(template_id, res_ids, fields=fields)
        
        if isinstance(res_ids, (int, long)):
            res_ids = [res_ids]
        res_ids_to_templates = super(MailTemplate, self).get_email_template_batch(template_id, res_ids)
