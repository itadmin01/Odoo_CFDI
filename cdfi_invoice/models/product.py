...
class ProductTemplate(models.Model):
    ...
    clave_unidad = fields.Char(string='Clave unidad', compute='_compute_clave_unidad', store=True)
    
    @api.depends('unidad_medida')
    def _compute_clave_unidad(self):
        for record in self:
            if record.unidad_medida:
                record.clave_unidad = UM_CLAVO_MAP[record.unidad_medida]
