...
class AccountInvoice(models.Model):
    ...
    def to_json(self):
        ...
        amount_untaxed = 0.0
        tax_grouped = {}
        items = {'numerodepartidas': len(self.invoice_line)}
        invoice_lines = []
        for line in self.invoice_line:
            if line.quantity < 0:
                continue
            price = line.price_unit * (1 - (line.discount or 0.0) / 100.0)
            amounts = line.invoice_line_tax_id.compute_all(price, line.quantity, product=line.product_id,
                                                         partner=line.invoice_id.partner_id)
        
            price_exclude_tax = amounts['total']
    ...
    def action_cfdi_generate(self):
        ...
        for invoice in self:
            if invoice.estado_factura == 'factura_correcta':
                raise ValidationError(_('Error para timbrar factura, Factura ya generada.'))
            if invoice.estado_factura == 'factura_cancelada':
                raise ValidationError(_('Error para timbrar factura, Factura ya generada y cancelada.'))
    ...
    def action_cfdi_cancel(self):
         for invoice in self:
            if invoice.factura_cfdi:
                if invoice.estado_factura == 'factura_cancelada':
                    pass
                    # raise ValidationError(_('La factura ya fue cancelada, no puede volver a cancelarse.'))
                if not invoice.company_id.archivo_cer:
                    raise ValidationError(_('Falta la ruta del archivo .cer'))
                archivo_cer_file = open(invoice.company_id.archivo_cer, 'rb').read()
                if not invoice.company_id.archivo_key:
                    raise ValidationError(_('Falta la ruta del archivo .key'))
